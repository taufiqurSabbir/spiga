...
class _Dial extends StatefulWidget {
   _Dial({
    ...
    required this.allowTimeFrom,
    required this.allowTimeTo,
    required this.selectedTimeAtThetaChange,
  }) : assert(selectedTime != null),
       assert(mode != null),
       assert(use24HourDials != null);
   ...
   final TimeOfDay selectedTime;
  TimeOfDay selectedTimeAtThetaChange;
  final _TimePickerMode mode;
  final bool use24HourDials;
  final ValueChanged<TimeOfDay>? onChanged;
  final VoidCallback? onHourSelected;
  final String allowTimeFrom;
  final String allowTimeTo;
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  void _updateThetaForPan({ bool roundMinutes = false }) {
    setState(() {
      ...
      if(_getTimeForTheta(angle).hour >= int.parse(widget.allowTimeFrom.split(":")[0]) &&
          _getTimeForTheta(angle).hour <= int.parse(widget.allowTimeTo.split(":")[0])
      ) {

        if(_getTimeForTheta(angle).hour == int.parse(widget.allowTimeFrom.split(":")[0])){
          if(_getTimeForTheta(angle).minute >= int.parse(widget.allowTimeFrom.split(":")[1])){
            _thetaTween
              ..begin = angle
              ..end = angle; // The controller doesn't animate during the pan gesture.
          }
        }
        if(_getTimeForTheta(angle).hour == int.parse(widget.allowTimeTo.split(":")[0])){
          if(_getTimeForTheta(angle).minute <= int.parse(widget.allowTimeTo.split(":")[1])){
            _thetaTween
              ..begin = angle
              ..end = angle; // The controller doesn't animate during the pan gesture.
          }
      ...
      // widget.selectedTimeAtThetaChange = _getTimeForTheta(angle);
      // widget.onChanged!(widget.selectedTimeAtThetaChange);
  ...
  void _selectHour(int hour) {
    ...
    if (widget.mode == _TimePickerMode.hour && widget.use24HourDials) {
      if(hour >= int.parse(widget.allowTimeFrom.split(":")[0]) &&
          hour <= int.parse(widget.allowTimeTo.split(":")[0])
      ) {
  ...
  void _selectMinute(int minute) {
    if(widget.selectedTime.hour == int.parse(widget.allowTimeFrom.split(":")[0])){
      if (minute >= int.parse(widget.allowTimeFrom.split(":")[1])) {
    ...
    else if(widget.selectedTime.hour == int.parse(widget.allowTimeTo.split(":")[0])){
      if (minute <= int.parse(widget.allowTimeTo.split(":")[1])) {
        _announceToAccessibility(context, localizations.formatDecimal(minute));
        final TimeOfDay time = TimeOfDay(
          hour: widget.selectedTime.hour,
          minute: minute,
    ...
    else if(widget.selectedTime.hour != int.parse(widget.allowTimeFrom.split(":")[0]) &&
        widget.selectedTime.hour != int.parse(widget.allowTimeTo.split(":")[0])
    ){
      _announceToAccessibility(context, localizations.formatDecimal(minute));
      final TimeOfDay time = TimeOfDay(
        hour: widget.selectedTime.hour,
  ...
  _TappableLabel _buildTappableLabel(TimeOfDay selectedTime, String mode, TextTheme textTheme, Color color, int value, String label, VoidCallback onTap) {

    //print(value);
    // print(widget.allowTimeFrom.split(":")[0]);
    // print(value >= int.parse(widget.allowTimeFrom.split(":")[0]));
    // print("------------------------------------------------");
  if(mode == 'h') {
    if (value >= int.parse(widget.allowTimeFrom.split(":")[0]) &&
        value <= int.parse(widget.allowTimeTo.split(":")[0])) {
      final TextStyle style = textTheme.bodyLarge!.copyWith(color: color);
      final double labelScaleFactor = math.min(MediaQuery
          .of(context)
          .textScaleFactor, 2.0);
      ...
      final TextStyle style = textTheme.bodyLarge!.copyWith(
    ...
    if(selectedTime.hour == int.parse(widget.allowTimeFrom.split(":")[0])){
      if (value >= int.parse(widget.allowTimeFrom.split(":")[1])) {
        final TextStyle style = textTheme.bodyLarge!.copyWith(color: color);
        final double labelScaleFactor = math.min(MediaQuery
            .of(context)
            .textScaleFactor, 2.0);
    ...
    else if(selectedTime.hour == int.parse(widget.allowTimeTo.split(":")[0])){
      if (value <= int.parse(widget.allowTimeTo.split(":")[1])) {
        final TextStyle style = textTheme.bodyLarge!.copyWith(color: color);
        final double labelScaleFactor = math.min(MediaQuery
            .of(context)
            .textScaleFactor, 2.0);
...
class TimePickerDialog extends StatefulWidget {
  ...
  /// Time range started time
  final String allowTimeFrom;

  /// Time range ended time
  final String allowTimeTo;

   TimeOfDay? selectedTimeTheta;
...
class _TimePickerDialogState extends State<TimePickerDialog> with RestorationMixin {
  ...
  void _announceInitialTimeOnce() {
   ...
   _handleTimeChanged(TimeOfDay valueOnSelect) {
    // print(valueOnSelect);
    _vibrate();
    setState(() {
      //_selectedTime.value = valueOnSelect;

      if(valueOnSelect.hour >= int.parse(widget.allowTimeFrom.split(":")[0]) &&
          valueOnSelect.hour <= int.parse(widget.allowTimeTo.split(":")[0])
      ) {
        if (valueOnSelect.hour == int.parse(widget.allowTimeFrom.split(":")[0])) {
          if (valueOnSelect.minute >= int.parse(widget.allowTimeFrom.split(":")[1])) {
            _selectedTime.value = valueOnSelect;
          }
        }
        else if (valueOnSelect.hour == int.parse(widget.allowTimeTo.split(":")[0])) {
          if (valueOnSelect.minute <= int.parse(widget.allowTimeTo.split(":")[1])) {
            _selectedTime.value = valueOnSelect;
          }
        }
        else{
          if (valueOnSelect.hour != int.parse(widget.allowTimeFrom.split(":")[0]) &&
              valueOnSelect.hour != int.parse(widget.allowTimeFrom.split(":")[0])
          ) {
            _selectedTime.value = valueOnSelect;
          }
  ...
  void _handleHourSelected() {
    ...
    setState(() {
        ...
        if (selectedTime.value.hour >=
            int.parse(widget.allowTimeFrom.split(":")[0]) &&
            selectedTime.value.hour <=
                int.parse(widget.allowTimeTo.split(":")[0])

        ) {
          _mode.value = _TimePickerMode.minute;
        }
        // else{
        //   //_mode.value = _TimePickerMode.hour;
        // }
          ...
          child: ExcludeSemantics(
            child: AspectRatio(
              ...
              child: _Dial(
                ...
                use24HourDials: use24HourDials,
                selectedTime: _selectedTime.value,
                selectedTimeAtThetaChange: _selectedTime.value,
                onChanged: _handleTimeChanged,
                onHourSelected: _handleHourSelected,
                allowTimeFrom: widget.allowTimeFrom,
                allowTimeTo: widget.allowTimeTo,
              ),
            ),
          ),
        );
...
Future<TimeOfDay?> myShowTimePicker({
  ...
  Offset? anchorPoint,
  required String allowTimeFrom,
  required String allowTimeTo
}) async {
  assert(context != null);
  assert(initialTime != null);
  ...
  final Widget dialog = TimePickerDialog(
    ...
    allowTimeFrom: allowTimeFrom,
    allowTimeTo:allowTimeTo
  );
